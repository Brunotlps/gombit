...
- **[REL-9] Scaffolded apps must resolve a published framework version** — `scaffold/templates/go.mod.tmpl` hardcodes `require github.com/LAA-Software-Engineering/gombit v0.0.0`, a version that does not exist. Today `go build ./...` in a fresh `gombit new` tree fails with "missing go.sum entry" unless the user adds a local `replace`; `goldentest` only passes because it injects one into a temp copy. **This blocks the documented quickstart.** Proposal: `gombit new` emits the CLI's own released version (`cli.Version`) as the requirement, falling back to `v0.0.0` + a documented `replace` for `dev` builds. `goldentest` already normalizes the require line back to `v0.0.0` before diffing, so committed goldens should be unaffected — confirm. AC: after `gombit new`, `go build ./...` succeeds with no manual edits, against a published tag. deps: REL-4 (first published tag). size: M. labels: `cli`, `generator`, `devx`.
