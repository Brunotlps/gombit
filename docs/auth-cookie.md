...
[Bearer JWT default](auth.md) (C3 / D3). Select it with `gombit new --auth
...
`Config.Auth.CookieSameSite` (see [`auth/cookie.go`](../auth/cookie.go)).
...
CSRF token). See [`docs/auth.md`](auth.md#config) for those and the
