...
6. **Build matrix** — `-trimpath`,
   ...
   > **Resolved during implementation — `CGO_ENABLED=0` is not viable.**
   > `database` imports `gorm.io/driver/sqlite` → `mattn/go-sqlite3` (cgo), and
   > `cmd/gombit` links it. A `CGO_ENABLED=0` binary *compiles and runs*, then
   > fails at the first SQLite connection with `go-sqlite3 requires cgo to
   > work` — confirmed by running `gombit doctor` from such a build. Since
   > SQLite is the default for `gombit new`, the release matrix builds on
   > **native runners with `CGO_ENABLED=1`** (one runner per target) instead of
   > cross-compiling from one Linux box.
...
| Risk | Mitigation |
| --- | --- |
| ~~`CGO_ENABLED=0` cross-compile fails if the sqlite driver is cgo-based~~ | **Confirmed and resolved.** The failure is at runtime, not build time. Release builds on native runners with `CGO_ENABLED=1`; the constraint is documented in `installation.md` and `releasing.md`. Follow-up worth considering: moving to `glebarez/sqlite` (pure Go) would collapse the matrix back to one cross-compiling job and remove the C-toolchain requirement from `go install` — a runtime dependency change, so out of scope here |
