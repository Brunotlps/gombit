...
`benchmarks/results/latest/` and validated in CI against drift — do not edit it
by hand. **Read [the methodology](benchmarks/docs/methodology.md), especially
...
### PostgreSQL CRUD read — `GET /api/projects`

Median requests/sec across trials, per concurrency (higher is better).

_Not yet recorded — run `make benchmark-crud-all`._

### Operational footprint

Container-start cold start (median), memory, and CPU under load; lower is better.
